...
export default function App() {
  ...
  // Reset/Clear everything back to raw defaults
  const handleResetWorkspace = () => {
    if (confirm("Are you sure you want to reset your entire digital workspace? This will restore the original demo models and overwrite any changes. Please download your workspace as a JSON file first if you wish to keep them!")) {
      localStorage.removeItem("adept_experiences_data");
      ...
      <div className="print:hidden flex flex-col flex-grow">
        ...
        <header id="workspace-master-header" className="bg-slate-900 text-white shadow-md sticky top-0 z-30">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-3 text-xs">
              ...
              <button
                id="header-reset-trigger"
                onClick={handleResetWorkspace}
                className="px-2.5 py-1.5 bg-slate-800 hover:bg-rose-950 hover:text-rose-200 text-slate-400 rounded-lg font-semibold flex items-center gap-1.5 transition-colors border border-slate-700/50"
                title="Reset local storage database"
              >
        ...
        <main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
          ...
          {currentExperience ? (
            ...
            <div className="text-center p-12 bg-white rounded-xl border border-dashed border-slate-200 shadow-2xs space-y-4">
              <AlertCircle className="w-12 h-12 text-slate-300 mx-auto" />
              <div className="space-y-1">
                <h3 className="font-semibold text-slate-800 text-sm">No evaluation workspaces exist</h3>
                <p className="text-xs text-slate-500">Create a brand new experience deconstruction model below to begin.</p>
              </div>
              <button
                id="empty-state-add-experience-btn"
                onClick={() => handleAddNewExperience("My Academic Research Assistant", "Internship")}
